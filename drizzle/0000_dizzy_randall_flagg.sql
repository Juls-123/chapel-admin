...
CREATE INDEX "idx_exeats_period_gist" ON "exeats" USING gist ("period" range_ops) WHERE (status = 'active'::text);--> statement-breakpoint
...
CREATE VIEW "public"."vw_recent_admin_actions" AS (SELECT aa.id, aa.action, aa.created_at, aa.object_type, aa.object_label, concat(a.first_name, ' ', a.last_name) AS admin_name FROM admin_actions aa JOIN admins a ON aa.admin_id = a.id ORDER BY aa.created_at DESC LIMIT 50);
*/