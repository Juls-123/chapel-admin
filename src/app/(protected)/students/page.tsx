...
import { StudentsUploadHistory } from "@/components/StudentsUploadHistory";
import { StudentsIssuesTab } from "@/components/StudentsIssuesTab";
import {
  UploadProgress,
  UploadProgressData,
...
export default function StudentManagementPage() {
      ...
      reader.onload = async (e) => {
        try {
          ...
          parsed.forEach((student, index) => {
            ...
            if (student.level < 100 || student.level > 500)
              errors.push(`Invalid level: ${student.level}`);
            if (!["male", "female"].includes(student.gender))
              errors.push(`Invalid gender: ${student.gender}`);
            if (student.email && !student.email.includes("@"))
              errors.push(`Invalid email format: ${student.email}`);
            if (student.parent_email && !student.parent_email.includes("@"))
              errors.push(
                `Invalid parent email format: ${student.parent_email}`
              );
            if (student.parent_phone && student.parent_phone.length < 10)
              errors.push(`Invalid parent phone: ${student.parent_phone}`);
  ...
  const onBulkSubmit = async () => {
    if (!isSuperAdmin || parsedData.length === 0 || !files[0]) {
      return;
    }
    ...
    try {
      // Use the hook's upload workflow method
      await uploadStudentsWithFile(
        files[0],  // File object
        parsedData as CreateStudentData[]  // Student data
      );
    } catch (error: any) {
      ...
      >
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(onIndividualSubmit)}
                className="space-y-4"
              >
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          type="email"
                          placeholder="e.g. john.doe@mtu.edu.ng"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                  ...
                  {parsedData.length > 0 && (
                    <div>
                      <h4 className="font-medium mb-2">
                        File Preview ({parsedData.length} students):
                      </h4>
                          ...
                          {parsedData.map((student, index) => (
                            <li key={index} className="p-2 text-sm">
                              ...
                              <p className="text-muted-foreground">
                                {student.matric_number} - {student.email}
                              </p>
                      ...
                      >
                        {isCreating
                          ? "Registering..."
                          : `Register ${parsedData.length} Students`}
                      </Button>
                    </div>
                  )}
                </CardContent>
