...
export default function ExeatManagerPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    limit: 10,
    search: "",
    status: undefined as 'active' | 'past' | 'canceled' | undefined,
  });
  ...
  const handleStatusFilter = useCallback((status: 'active' | 'past' | 'canceled' | undefined) => {
  ...
  if (exeatsError) {
    return (
      <AppShell>
        ...
        <ErrorState
          message={
            exeatsError.message || "An error occurred while loading exeats"
          }
          onRetry={() => refetchExeats()}
        />
      </AppShell>
    );
  }
    ...
    <AppShell>
      ...
      >
        <div className="flex gap-2">
          ...
          >
            <RefreshCw
              className={cn("h-4 w-4 mr-2", exeatsLoading && "animate-spin")}
            />
            Refresh
          </Button>
